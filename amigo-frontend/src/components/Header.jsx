...
import {
  FaVideo, FaChevronDown, FaCog, FaSignOutAlt,
        ...
        <div className="flex items-center h-16 gap-8">
          ...
          <button onClick={() => navigate('/dashboard')}
            ...
            <div className="w-8 h-8 rounded-xl bg-gradient-sage flex items-center justify-center
                           shadow-sage-sm group-hover:shadow-sage-md transition-shadow duration-200">
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <button className="hidden sm:flex btn-primary text-xs px-3 py-2 gap-1.5"
            ...
            <div className="relative" ref={dropdownRef}>
              <button onClick={() => setMenuOpen(p => !p)}
                ...
                <div className="hidden sm:flex flex-col items-start leading-none">
                  <span className="text-xs font-semibold text-charcoal-800">{user?.fullName?.split(' ')[0] || 'User'}</span>
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-2 w-52 bg-beige-50 border border-beige-300
                                rounded-2xl shadow-card-hover py-1.5 animate-scale-in z-50">
                  ...
                  <div className="py-1">
                    <button onClick={() => { setMenuOpen(false); navigate('/profile'); }}
                      className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-charcoal-700
                                 hover:bg-beige-100 hover:text-sage-700 transition-colors">
            ...
            {NAV_LINKS.map(({ label, path }) => (
              <button key={path}
                ...
                className={[
                  ...
                  isActive(path) ? 'bg-sage-100 text-sage-700 font-semibold' : 'text-charcoal-600 hover:bg-beige-200',
                ].join(' ')}>
                {label}
              </button>
            ))}
